...
#Region "Insert the page function here"
    ...
    Private Sub Page1()
        ...
        Try
            ...
            'azham 22-feb-2016 ====================================================
            strSQL = "select taxp_profile.tp_name," & _
                "(taxp_profile.tp_ref_no_prefix + taxp_profile.tp_ref_no1 + taxp_profile.tp_ref_no2 + taxp_profile.tp_ref_no3)," & _
                "(taxp_profile.tp_ic_new_1 + taxp_profile.tp_ic_new_2 + taxp_profile.tp_ic_new_3)," & _
                "taxp_profile.tp_passport_no," & _
                "taxp_profile.tp_country," & _
                "taxp_profile.tp_gender," & _
                "taxp_profile.tp_status," & _
                "taxp_profile.tp_date_marriage," & _
                "taxp_profile.tp_date_divorce," & _
                "taxp_profile.tp_type_assessment," & _
                "taxp_profile.tp_hw_name," & _
                "(taxp_profile.tp_hw_ic_new1 + taxp_profile.tp_hw_ic_new2 + taxp_profile.tp_hw_ic_new3)," & _
                "taxp_profile.tp_hw_passport_no," & _
                "taxp_profile.tp_assessmenton, " & _
                "(taxp_profile.tp_curr_add_line1 + ', ' + taxp_profile.tp_curr_add_line2 + ', ' + taxp_profile.tp_curr_add_line3 + ', '+ taxp_profile.tp_curr_postcode + ' ' + taxp_profile.tp_curr_city + ', ' + taxp_profile.tp_curr_state)," & _
                "taxp_profile2.tp_ref_No," & _
                "taxp_profile2.TP_DOB," & _
            ...
            If ds.Tables(0).Rows.Count > 0 Then
                ...
                If Not IsDBNull(ds.Tables(0).Rows(0).Item(16)) Then
                    If Not String.IsNullOrEmpty(ds.Tables(0).Rows(0).Item(16).ToString) Then
                        pdfFormFields.SetField(pdfFieldPath & "ZK3[0]", ds.Tables(0).Rows(0).Item(16).ToString)
                    End If
                End If

                If Not IsDBNull(ds.Tables(0).Rows(0).Item(16)) Then
                    If Not String.IsNullOrEmpty(ds.Tables(0).Rows(0).Item(16).ToString) Then
                        pdfFormFields.SetField(pdfFieldPath & "ZK4[0]", Format(CDate(ds.Tables(0).Rows(0).Item(16).ToString), "dd-MM-yyyy"))
                    End If
                End If

                If Not IsDBNull(ds.Tables(0).Rows(0).Item(17)) Then
                    If Not String.IsNullOrEmpty(ds.Tables(0).Rows(0).Item(17).ToString) Then
                        pdfFormFields.SetField(pdfFieldPath & "ZK5[0]", Format(CDate(ds.Tables(0).Rows(0).Item(17).ToString), "dd-MM-yyyy"))
                    End If
                End If
                ...
                If dr.Read() Then
                    ...
                    If Not IsDBNull(dr("TC_TAX_FIRST_TAX")) Then
                        ...
                        pdfFormFields.SetField(pdfFieldPath & "B11a_2[0]", FormatFixedAmount(Left(CStr(dr("TC_TAX_FIRST_TAX")), (Len(CStr(dr("TC_TAX_FIRST_TAX"))) - 3)))) 'FormatFixedAmount(Left(tmpDec.ToString("N0")), (Len(tmpDec.ToString("N0")) - 3))))
                    End If
                    ...
                    If Not IsDBNull(dr("TC_TAX_BALANCE_RATE")) Then
                        If Len(dr("TC_TAX_BALANCE_RATE")) > 1 Then
                            '     Dim tmpDec As Decimal = IIf(IsNumeric(dr("TC_TAX_FIRST_TAX")), dr("TC_TAX_FIRST_TAX"), 0)
                            pdfFormFields.SetField(pdfFieldPath & "B11b_2[0]", dr("TC_TAX_BALANCE_RATE"))
                            ' pdfFormFields.SetField(pdfFieldPath & "B11b_2[0]", Mid(CStr(dr("TC_TAX_BALANCE_RATE")), 1, 1) & Mid(CStr(dr("TC_TAX_BALANCE_RATE")), 2, 3))
                        Else
    ...
    Private Sub Page2()
        ...
        Try
            ...
            'azham 22-feb-2016 ====================================================
            If ds.Tables(0).Rows.Count > 0 Then
                If Not IsDBNull(ds.Tables(0).Rows(0).Item(2)) And Not IsDBNull(ds.Tables(0).Rows(0).Item(3)) Then
                    pdfFormFields.SetField(pdfFieldPath & "ZK3[0]", ds.Tables(0).Rows(0).Item(2) & "-" & ds.Tables(0).Rows(0).Item(3))
                Else
                    pdfFormFields.SetField(pdfFieldPath & "ZK3[0]", " ")
                End If
            End If
            'azham 22-feb-2016 ====================================================
            '--- D1 ---
            ...
            dr2 = datHandler.GetDataReader("select TP_MOTHER_IC,TP_FATHER_IC,TP_DISPOSAL1976,TP_LDMN_DISPOSAL from taxp_profile2 where tp_ref_no= '" & pdfForm.GetRefNo & "'")
            If dr2.Read() Then
                        ...
                        If Not String.IsNullOrEmpty(dr2("tcc_amount")) Then
                            ...
                            Select Case dr2("tcc_key")
                                ...
                                Case 26
                                    ...
                                    tmpTotal += dr2("tcc_amount")
                                Case 27
                                    pdfFormFields.SetField(pdfFieldPath & "D7A_7[0]", FormatFixedAmount(dr2("tcc_amount").ToString))
                                    tmpTotal += dr2("tcc_amount")
                                Case 28
                ...
                For i As Integer = 1 To 5
                    Select Case i
                        Case 1
                            ...
                            pdfFormFields.SetField(pdfFieldPath & "F16a_2[0]", intArrayChild100(i) * 2000)
                            pdfFormFields.SetField(pdfFieldPath & "F16a_3[0]", intArrayChild50(i).ToString)
                            pdfFormFields.SetField(pdfFieldPath & "F16a_4[0]", intArrayChild50(i) * 1000)
                        Case 2
                            pdfFormFields.SetField(pdfFieldPath & "F16b_1[0]", intArrayChild100(i).ToString)
                            pdfFormFields.SetField(pdfFieldPath & "F16b_2[0]", intArrayChild100(i) * 2000)
                            pdfFormFields.SetField(pdfFieldPath & "F16b_3[0]", intArrayChild50(i).ToString)
                            pdfFormFields.SetField(pdfFieldPath & "F16b_4[0]", intArrayChild50(i) * 1000)
                        Case 3
                            pdfFormFields.SetField(pdfFieldPath & "F16b_5[0]", intArrayChild100(i).ToString)
                            pdfFormFields.SetField(pdfFieldPath & "F16b_6[0]", intArrayChild100(i) * 8000)
                            pdfFormFields.SetField(pdfFieldPath & "F16b_7[0]", intArrayChild50(i).ToString)
                            pdfFormFields.SetField(pdfFieldPath & "F16b_8[0]", intArrayChild50(i) * 4000)
                        Case 4
                            pdfFormFields.SetField(pdfFieldPath & "F16c_1[0]", intArrayChild100(i).ToString)
                            pdfFormFields.SetField(pdfFieldPath & "F16c_2[0]", intArrayChild100(i) * 6000)
                            pdfFormFields.SetField(pdfFieldPath & "F16c_3[0]", intArrayChild50(i).ToString)
                            pdfFormFields.SetField(pdfFieldPath & "F16c_4[0]", intArrayChild50(i) * 3000)
                        Case 5
                            pdfFormFields.SetField(pdfFieldPath & "F16c_5[0]", intArrayChild100(i).ToString)
                            pdfFormFields.SetField(pdfFieldPath & "F16c_6[0]", intArrayChild100(i) * 14000)
                            pdfFormFields.SetField(pdfFieldPath & "F16c_7[0]", intArrayChild50(i).ToString)
                            pdfFormFields.SetField(pdfFieldPath & "F16c_8[0]", intArrayChild50(i) * 7000)
                    End Select
