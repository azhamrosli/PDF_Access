...
#Region "Insert the page function here"
    ...
    Private Sub Page1()
        ...
        Try
            ...
            '"TAXP_PROFILE.TP_REF_NO_PREFIX," & _
            '"(TAXP_PROFILE.TP_REF_NO1 + TAXP_PROFILE.TP_REF_NO2 + TAXP_PROFILE.TP_REF_NO3)," & _
            '"(TAXP_PROFILE.TP_IC_NEW_1 + TAXP_PROFILE.TP_IC_NEW_2 + TAXP_PROFILE.TP_IC_NEW_3)," & _
            '"TAXP_PROFILE.TP_IC_OLD," & _
            '"TAXP_PROFILE.TP_POLICE_NO," & _
            '"TAXP_PROFILE.TP_ARMY_NO," & _
            '"TAXP_PROFILE.TP_PASSPORT_NO," & _
            '"TAXP_PROFILE.TP_PASSWPORTDUEDATE," & _
            '"TAXP_PROFILE.TP_RESIDENCE," & _
            ...
            '"TAXP_PROFILE.TP_KUP," & _
            '"(TAXP_PROFILE.TP_CURR_ADD_LINE1 + ', ' + TAXP_PROFILE.TP_CURR_ADD_LINE2 + ', ' + TAXP_PROFILE.TP_CURR_ADD_LINE3) AS ADDRESS," & _
            '"TAXP_PROFILE.TP_CURR_POSTCODE," & _
            '"TAXP_PROFILE.TP_CURR_CITY," & _
            '"TAXP_PROFILE.TP_CURR_STATE," & _
            '"TAXP_PROFILE.TP_ASSESSMENTON," & _
            '"TAXP_PROFILE2.TP_REF_NO," & _
            '"TAXP_PROFILE2.TP_HW_DOB " & _
            '"FROM TAXP_PROFILE " & _
            '"INNER JOIN TAXP_PROFILE2 on (TAXP_PROFILE.TP_REF_NO1 + TAXP_PROFILE.TP_REF_NO2 + TAXP_PROFILE.TP_REF_NO3)=TAXP_PROFILE2.TP_REF_NO " & _
    ...
    Private Sub Page2()
        ...
        Try
            ...
            If ds.Tables(0).Rows(0).Item(12).ToString() <> "" Then
                pdfFormFields.SetField(pdfFieldPath & "A15[0]", ds.Tables(0).Rows(0).Item(12).ToString)
            Else
            ...
            If ds.Tables(0).Rows(0).Item(13).ToString() <> "" Then
                ...
                pdfFormFields.SetField(pdfFieldPath & "A16_1[0]", strArray(0))
                pdfFormFields.SetField(pdfFieldPath & "A16_2[0]", strArray(1))
            Else
                pdfFormFields.SetField(pdfFieldPath & "A16_1[0]", "---")
                pdfFormFields.SetField(pdfFieldPath & "A16_2[0]", "---")
            End If
            If ds.Tables(0).Rows(0).Item(14).ToString() <> "" Then
                pdfFormFields.SetField(pdfFieldPath & "A17[0]", ds.Tables(0).Rows(0).Item(14).ToString)
            Else
                pdfFormFields.SetField(pdfFieldPath & "A17[0]", "---")
            End If
            ...
            'Master Data
            prmOledb(0) = New OleDbParameter("@ref_no", pdfForm.GetRefNo)
            ds = datHandler.GetData("SELECT TP_HW_LAST_PASSPORT_NO, TP_HW_DOB FROM TAXP_PROFILE2 WHERE" _
                        & " TP_REF_NO= ?", prmOledb)
            pdfFormFields.SetField(pdfFieldPath & "B7a[0]", ds.Tables(0).Rows(0).Item(0).ToString)
            If Not IsDBNull(ds.Tables(0).Rows(0).Item(1)) Then
                pdfFormFields.SetField(pdfFieldPath & "B9[0]", FormatDate(ds.Tables(0).Rows(0).Item(1)))
            Else
                pdfFormFields.SetField(pdfFieldPath & "B9[0]", "")
            End If
        ...
        Try
            ...
            If intNumberRecord <= 2 Then
                If dblRentalIncome > 0 Then
                    pdfFormFields.SetField(pdfFieldPath & "C3_2[0]", CDbl(FormatNumber((dblRentalIncome.ToString), 0)))
                End If
            Else
                    ...
                    If Not IsDBNull(dr2("TP_HW_TYPEOFINCOME")) And Not String.IsNullOrEmpty(dr2("TP_HW_TYPEOFINCOME").ToString) Then
                        If dr2("TP_TYPE_ASSESSMENT") = "1" Then
                            If (dr2("TP_GENDER") = "1" And dr2("TP_ASSESSMENTON") = "1") Or _
                                ...
                                pdfFormFields.SetField(pdfFieldPath & "C35_1[0]", CDbl(FormatNumber((dr2("TP_HW_TYPEOFINCOME").ToString), 0)))
                            Else
    ...
    Private Sub Page5()
        ...
        Try
            ...
            pdfFormFields.SetField(pdfFieldPath & "D8[0]", CDbl(FormatNumber(ds.Tables(0).Rows(0).Item(5).ToString, 2).ToString.Replace(".", "")))
            pdfFormFields.SetField(pdfFieldPath & "D8A[0]", CDbl(FormatNumber(ds.Tables(0).Rows(0).Item(10).ToString, 2).ToString.Replace(".", "")))
            pdfFormFields.SetField(pdfFieldPath & "D9[0]", CDbl(FormatNumber(ds.Tables(0).Rows(0).Item(6).ToString, 2).ToString.Replace(".", "")))
            pdfFormFields.SetField(pdfFieldPath & "D10[0]", CDbl(FormatNumber(ds.Tables(0).Rows(0).Item(7).ToString, 2).ToString.Replace(".", "")))
            pdfFormFields.SetField(pdfFieldPath & "D11[0]", CDbl(FormatNumber(ds.Tables(0).Rows(0).Item(8).ToString, 2).ToString.Replace(".", "")))
    ...
    Private Sub Page6()
        ...
        Try
            ...
            ds = datHandler.GetData("SELECT TC_AL_CY_UNASORBED_LOSS, TC_AL_BAL_UNASORBED_LOSS, TC_AL_BALANCE_CF," _
                            ...
                            & " FROM TAX_COMPUTATION WHERE TC_REF_NO=? AND TC_YA=?", prmOledb(0), prmOledb(1))
            pdfFormFields.SetField(pdfFieldPath & "H1a[0]", CDbl(FormatNumber(ds.Tables(0).Rows(0).Item(0).ToString, 0)))
    ...
    Private Sub Page10()
        ...
        Try
            ...
            'Tax Agent
            ReDim strArray(1)
            dr = datHandler.GetDataReader("SELECT * FROM [TAXA_PROFILE]")
            If dr.Read() Then
